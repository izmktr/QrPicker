...
7. 複合インデックスの作成方法
   ...
   1. アプリを実行して履歴画面を開き、インデックス不足のエラーが出たら、エラーメッセージ内のリンクをクリックします。
   2. Firebase Console の Firestore Database 画面で「インデックスを作成」が開くので、そのまま作成します。
   3. 手動で作成する場合は、Firestore Database → インデックス → 複合インデックス → インデックスを追加 を開きます。
   4. 対象コレクションは `scanHistory` を指定します。
   5. フィールドは次の順で追加します。
      - `userId` / 昇順
      - `deleted` / 昇順
      - `deletedAt` / 昇順
   6. 保存して作成を待ちます。反映まで数分かかることがあります。

   この設定は、削除済みデータを 30 日より古いものから整理するための検索に使います。
